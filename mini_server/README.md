...
`mini_server/` 是宽松接收器：它适合快速确认公网链路、HTTP POST 和 Socket.IO 推送是否正常。主系统当前的新协议 payload 可以直接发给它，但它只做基础接收与展示，不做主系统那样的严格风机字段校验；需要严格校验时请使用 `mini_one_node/` 或主系统 `code/`。
...
- `node_id`：节点名称
- `sub`：本帧风机数量
- `001..NNN`：连续风机编号，每台风机为 `[voltage, current, speed, temperature]`

### 3）兼容旧版 WindSight 32 通道格式（可选）
