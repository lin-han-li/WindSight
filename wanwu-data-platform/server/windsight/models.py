...
class TelemetryRecord(db.Model):
    """A source-agnostic JSON telemetry frame.

    Wind-turbine packets keep using ``NodeUpload``/``TurbineMeasurement`` for
    backwards compatibility.  Every other JSON object accepted by ``/upload``
    is stored here together with a normalized set of numeric fields in
    ``TelemetryMetric``.
    """
    ...
    id = db.Column(db.Integer, primary_key=True)
    node_id = db.Column(db.String(100), nullable=False, index=True)
    source_field = db.Column(db.String(32), nullable=False)
    ...
    __table_args__ = (
        Index("ix_telemetry_records_node_id_timestamp", "node_id", "timestamp"),
    )

    def payload_dict(self):
        try:
            payload = json.loads(self.raw_payload) if self.raw_payload else {}
            return payload if isinstance(payload, dict) else {}
        except (TypeError, ValueError):
            return {}

    def metrics_dict(self, fields=None):
        wanted = set(fields) if fields else None
        return {
            metric.path: float(metric.value)
            for metric in self.metrics
            if wanted is None or metric.path in wanted
        }
    ...
    def to_dict(self, fields=None):
        ...
        return {
            ...
            "source_id": self.source_value,
            "timestamp": iso_beijing(self.timestamp, with_seconds=True, with_ms=True),
            "metrics": self.metrics_dict(fields),
            "payload": self.payload_dict(),
        }

    def to_event_dict(self):
        """Keep the real-time event shape intentionally small and stable."""
...
class TelemetryMetric(db.Model):
    """One finite numeric value extracted from a :class:`TelemetryRecord`."""

    __tablename__ = "telemetry_metrics"

    id = db.Column(db.Integer, primary_key=True)
    telemetry_record_id = db.Column(
        db.Integer,
        db.ForeignKey("telemetry_records.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    node_id = db.Column(db.String(100), nullable=False, index=True)
    ...
    record = relationship("TelemetryRecord", back_populates="metrics")

    __table_args__ = (
        UniqueConstraint("telemetry_record_id", "path", name="uq_telemetry_metric_record_path"),
        Index("ix_telemetry_metrics_node_path_timestamp", "node_id", "path", "timestamp"),
    )
