...
class ParsedUpload:
    ...
    def turbine_codes(self) -> list[str]:
        return list(self.turbines.keys())


def build_turbine_codes(count: int) -> list[str]:
    return [f"{index:03d}" for index in range(1, count + 1)]
...
def _parse_turbine_sample(code: str, value) -> TurbineSample:
